...
namespace WaterCloud.Application.SystemManage
{
    public class UserApp
    {
        ...
        {
            var cachedata = service.CheckCacheList(cacheKey + "list", CacheId.user);
            if (!string.IsNullOrEmpty(keyword))
            {
        ...
        public void SubmitUserForm(UserEntity userEntity)
        {
            ...
            redisCache.Remove(cacheKey + userEntity.F_Id, CacheId.user);
            redisCache.Remove(cacheKey + "list", CacheId.user);
        }
        ...
        {
            var cachedata = service.CheckCache(cacheKey, keyValue, CacheId.user);
            return cachedata;
        }
        public void DeleteForm(string keyValue)
        {
            service.DeleteForm(keyValue);
            redisCache.Remove(cacheKey + keyValue, CacheId.user);
            redisCache.Remove(cacheKey + "list", CacheId.user);
        }
        ...
        public UserEntity CheckLogin(string username, string password)
        {
            ...
            if (userEntity != null)
            {
                ...
                {
                    //缓存用户账户信息
                    var userLogOnEntity= redisCache.Read<OperatorUserInfo>(cacheKeyOperator + "info_" + userEntity.F_Id, CacheId.loginInfo);
                    if (userLogOnEntity==null)
                    {
                        ...
                        redisCache.Write<OperatorUserInfo>(cacheKeyOperator + "info_" + userEntity.F_Id, userLogOnEntity, CacheId.loginInfo);
                    }
                    if (userLogOnEntity == null)
                    {
                    ...
                    if (dbPassword == userLogOnEntity.F_UserPassword)
                    {
                        ...
                        redisCache.Remove(cacheKeyOperator + "info_" + userEntity.F_Id, CacheId.loginInfo);
                        redisCache.Write<OperatorUserInfo>(cacheKeyOperator + "info_" + userEntity.F_Id, userLogOnEntity, CacheId.loginInfo);
